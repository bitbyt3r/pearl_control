    ...
    with bus as client:
        if options.listen:
            ...
            for layout in layouts:
                if layout['name'] == "Passthrough":
                    passthrough_layout = layout['id']
                if layout['name'] == "Schedule":
                    schedule_layout = layout['id']
            if not passthrough_layout:
                sys.exit("Could not find layout called Passthrough on channel Projector")
            if not schedule_layout:
                sys.exit("Could not find layout called Schedule on channel Projector")
